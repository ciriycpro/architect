workspace "Compliance Assistant" "Автоматизация документооборота ИП Таирова — сверка договоров/счетов/актов, алерты по 3 типам проблем" {
    ...
    model {
        ...
        compliance = softwareSystem "Compliance Assistant" "Контур автоматической сверки документооборота на coo.gcp (DEC-0023, DEC-0025)" {
            ...
            summaryprep = container "Summary-prep v1.0" "Python 3.11 + FastAPI + asyncio + httpx, /opt/mail-stack/summary-prep/" "[НОВЫЙ СЕРВИС, DEC-0030 Accepted 23.06.2026] coo:8772. Дистилляция смысла длинных документов через двухуровневый map-reduce. Endpoint POST /distill принимает {text, metadata, quality_mode} → DistillResult (Pydantic JSON: document_type, summary_brief, key_facts, amounts, dates, parties, requirements, raw_excerpts, confidence). Порог DISTILL_CHAR_THRESHOLD=80000 chars (документы короче → as-is wrapped). MAP параллельный через asyncio.gather, REDUCE объединяет, VERIFY проверка галлюцинаций на первой странице исходника. LLM: Haiku 4.5 via OpenRouter (тот же стек что DEC-0009). Soft contract для саммари (MVP), Hard contract для compliance-logic в DEC-0028 Phase 2. FS-кэш в /var/lib/summary-prep/cache/<sha256[:2]>/<sha256>.json TTL 30 дней. См. DEC-0030"
        }
        ...
        # Summary-prep связи (DEC-0030)
        orchestrator -> summaryprep "POST /distill для каждого вложения после parser (workflow email_digest_v1 step 4, после parser, перед summary). Параллельно asyncio.gather на нескольких вложениях" "HTTP REST + X-API-Key"
        summaryprep -> openrouter "Map/Reduce/Verify через Haiku 4.5 (DEC-0009 stack reuse)" "HTTPS API"
        summaryprep -> summaryservice "DistillResult JSON присоединяется к каждому письму в массиве дайджеста (через orchestrator, не напрямую). На рисунке стрелка обозначает поток данных" "data flow (через orchestrator)"
