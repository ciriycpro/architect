workspace "Tairov Compliance Helper" "Автоматизация документооборота ИП Таирова — сверка договоров/счетов/актов, алерты по 3 типам проблем" {
    ...
    model {
        ...
        compliance = softwareSystem "Compliance Helper" "Контур автоматической сверки документооборота на coo.gcp" {
            ...
            # Прод (12-13.05.2026): mailservice, attachmentservice, parserservice
            # Planned: summaryservice, emaildigest
            ...
            summaryservice = container "Summary Service (planned)" "Python 3.11 + FastAPI + systemd, Claude Haiku 4.5" "[PLANNED — DEC-009] coo:8768. Endpoint /summary — текстовое саммари массива писем. Primary: Claude Haiku 4.5 через OpenRouter. Fallback: DeepSeek V3. ~25 руб/мес на 30 вызовов"
        ...
        parserservice -> openrouter "LLM-vision на JPG/PDF-сканы — Qwen3-VL + Qwen 2.5 VL (planned)" "HTTPS"
        summaryservice -> openrouter "Claude Haiku 4.5 для дайджеста (planned)" "HTTPS"
