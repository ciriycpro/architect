workspace "Tairov Compliance Helper" "Автоматизация документооборота ИП Таирова — сверка договоров/счетов/актов, алерты по 3 типам проблем" {
    ...
    model {
        ...
        compliance = softwareSystem "Compliance Helper" "Контур автоматической сверки документооборота на coo.gcp" {
            ...
            # Planned: orchestrator (Go custom v1 — DEC-014, реализуется 13.05.2026)
            ...
            orchestrator = container "Orchestrator v1 (Go custom)" "Go 1.21+ + net/http + robfig/cron + slog + caarlos0/env, /opt/mail-stack/orchestrator/" "[PLANNED — DEC-014, реализуется 13.05.2026] coo:8769. Triggers: Schedule (cron) + Webhook /digest-now + /check-mail. Координирует цепочку: mail → attachment → parser → summary → telegram + sheets. Структурированные logs (slog) с trace_id. Auth: X-API-Key. Rate limit token bucket. Secure by Design Уровень 0 встроен. Спроектирован для миграции на Temporal headless v2 → KAMF v3 (80%+ кода переиспользуется)"
        ...
        orchestrator -> messengers "Доставка summary_telegram (planned)" "API"
