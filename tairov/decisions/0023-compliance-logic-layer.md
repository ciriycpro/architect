...
Accepted (23.05.2026). Конкретное технологическое решение для **business-tier** (см. DEC-022, DEC-025). Реализация v1.0 в активной разработке (см. Implementation Notes v0.0.6-SNAPSHOT от 31.05.2026 — коммит 4 на проде).
...
## Implementation Notes (v0.0.6-SNAPSHOT, 31.05.2026) — Коммит 4: operation_class + Contract + Act + Reconciler (матч по основанию)

Прогресс кода: 30–31.05.2026. Весь коммит 4 **развёрнут на проде (coo) и провалидирован на реальных данных** — 547 операций из годовых выписок двух ИП (Таиров/ВТБ, Веретенникова/Альфа), загруженных ингестором через REST API. Reconciler матчит операцию на **конкретный договор по номеру (основание)**, имена контрагентов нормализуются на входе.

### Что реализовано в v0.0.6 (миграции 0013–0017)

| Блок | Файлы / суть |
|---|---|
| **operation_class** (0013) | `OperationClass` enum (OWN_TRANSFER/TAX/BANK_FEE/CARD_ACQUIRING/COUNTERPARTY) + поле на MoneyOperation + API. Вход для Reconciler — только COUNTERPARTY |
| **Contract** (0014) | `SigningStatus` enum + `Contract` (@Audited, спец. Document type=CONTRACT) + client/counterparty + `contract_number`/dates/subject/amount/signing_status. `ContractRepository.findByClientIdAndCounterpartyId` — ключ сверки. CRUD-эндпоинты |
| **Act** (0015) | `Act` (@Audited, спец. Document type=ACT) + contract FK. CRUD |
| **ReconciliationFlag + Reconciler + cron** (0016) | `ReconciliationFlagType` (MISSING_CONTRACT) + `ReconciliationFlagStatus` (DETECTED/REQUEST_SENT/RESOLVED) + entity @Audited. `ReconcilerService` + `ReconcilerScheduler` (@Scheduled daily 10:30). Эндпоинты reconcile / реестр / rescan |
| **Матч по основанию** (0017) | `contract_ref` на флаге (номер договора из назначения платежа). `ReconcilerService` парсит номер из purpose и матчит на конкретный Contract |
| **Нормализация имён** | `CounterpartyNameNormalizer` — ООО/ИП/АО/ПАО + чистка артефактов выписки. Применяется на создании контрагента + эндпоинт `/admin/counterparties/normalize` для существующих |

### Reconciler — логика (разжёвано, актуальная v1)

**Вход.** Операции класса `COUNTERPARTY` — реальные контрагенты, входящие и **исходящие**. Класс направления не различает. Исключены OWN_TRANSFER/BANK_FEE/CARD_ACQUIRING/TAX (договор не нужен).

**Основание = номер договора из назначения платежа.** Reconciler regex-ом тянет из `purpose` номер: «по договору № X от …» / «по Договору X от …». Важная деталь: `\w` в Java по умолчанию ASCII — для кириллицы обязателен флаг `UNICODE_CHARACTER_CLASS`, иначе `догов\w*` не доходит до номера. Номера у контрагентов часто «датовидные» (договор №210324 от 21.03.2024) — это **реальные номера**, не даты.

**Гранулярность — по (контрагент + номер договора), не по платежу и не по контрагенту.** У одного контрагента может быть несколько договоров под разными номерами — каждый недостающий = отдельный флаг. Платежи без распознанной ссылки на договор группируются в `contract_ref = null` («оплата по счёту / без основания»).

**Хранилище договоров = таблица `contracts`.** Reconciler проверяет её через `ContractRepository`. На текущем этапе таблица **пуста** (загружены только выписки, договоров ноль) → флагается всё. Система НЕ ходит сама за файлами договоров в GDrive/почту — это отдельный ингест (документ → Contract).

**Алгоритм `reconcile(client)`:**
1. Берём COUNTERPARTY-операции, тянем основание из purpose, группируем по (counterparty + ref) с агрегатом count/sum.
2. ref задан → ищем Contract с этим номером по контрагенту. Нет → флаг (`contract_ref=ref`, «договор N не найден»).
3. ref пуст → достаточно любого договора по контрагенту; нет ни одного → флаг (`contract_ref=null`).
4. Идемпотентно: открытый флаг по (client, counterparty, ref) обновляется, не дублируется.

**Жизненный цикл:** `DETECTED` → `REQUEST_SENT` (коммит 5, Agent Caller) → `RESOLVED`. Без Spring Statemachine (три состояния — enum + переходы в сервисе).

**Ежедневный re-scan (cron 10:30):** по открытым флагам — появился ли Contract именно с этим номером (для ref=null — любой по контрагенту) → `RESOLVED`. Реестр схлопывается сам по мере загрузки договоров.

**Реестр отсутствующих договоров = открытые MISSING_CONTRACT.** Колонка «договор N» означает «недостающий договор N» (платёж на него ссылается, а его нет), НЕ «договор существует».

### Нормализация имён контрагентов

Имена из выписки приходят с артефактами PDF-разбора: переносы строк дают лишние пробелы внутри слов (`ОТВЕТСТВЕННОСТ Ь`), утекает хвост расчётного счёта (`ООО "ЭТМ" Р/С 40702… в`), полная орг-форма вместо аббревиатуры. `CounterpartyNameNormalizer`: чинит разрывы, срезает хвост Р/С, сворачивает орг-форму в ООО/ИП/АО/ПАО/ЗАО. Применяется на входе в Registry + одноразовым прогоном по существующим. Цель — чтобы в реестр и в будущие алерты шло чистое `ООО "…"` / `ИП ФИО` + ИНН (ИНН — надёжный ключ).

### Production smoke (31.05.2026, реальные данные)

| Тест | Результат |
|---|---|
| Liquibase 0013–0017 (base + aud) | all ran successfully |
| Ингест 547 операций (ВТБ 486 + Альфа 61) через API | 2 клиента, 23 контрагента, 2 выписки, 547 операций |
| operation_class | OWN_TRANSFER 264 / BANK_FEE 173 / COUNTERPARTY 100 / TAX 10 = 547 (совпало с парсером) |
| Нормализация имён | 6 контрагентов очищено (ИП КУЛЕШОВ…, ООО "СТРОИТЕЛЬСТВО И ЛАНДШАФТНАЯ АРХИТЕКТУРА", ООО "ЭТМ") |
| reconcile Таиров (по основанию) | 18 флагов (по контрагент+договор) |
| reconcile Веретенникова | 8 флагов, отдельно |

**Что показал реестр:** МОНТАЖПРОЕКТ распался на два флага — **договор 12-111025** (25 оп, 28.8 млн, ссылка есть) и **без основания** (26 оп, 19.0 млн, платежи «по счёту»). ООО "СТРОИТЕЛЬСТВО И ЛАНДШАФТНАЯ АРХИТЕКТУРА" — 7 разных договоров (191024, 011024, 210424, 210324, 290524, 231124, 200424), каждый отдельным флагом. Это и есть точность по основанию, которой не было в counterparty-уровневой v0.

### Архитектурные решения этой сессии

- **Источники документов.** Почта (mail-stack) — **основной real-time канал** (Таиров шлёт выписки/договоры письмом). GDrive-качалка (rclone) — **backfill-инструмент** для разовой массовой заливки истории/договоров. Не основной поток, но нужен. Соответствует DEC-022/023 (mail-stack primary + BackfillService).
- **Границы Spring Boot.** Spring = **business-tier**: Registry + Inspector + Reconciler + статусы/сверка — логике тут место, разрастание в этих рамках ок. НЕ тащим в Spring: разбор документов (→ parser-service, Python), исходящие сообщения (→ Agent Caller), маршрутизацию (→ orchestrator, Go).

### Метрики прогресса

| Метрика | v0.0.5 | v0.0.6 | Δ |
|---|---|---|---|
| REST endpoints | 30 | 40 | +10 (contracts/acts/reconcile/normalize) |
| Service классов | 10 | 13 | +3 (Contract/Act/Reconciler) |
| Entity | 9 | 12 | +3 (Contract/Act/ReconciliationFlag) |
| Audit таблицы | 9 + revinfo | 12 + revinfo | +3 |
| БД таблицы (всего) | 19 | 25 | +6 |
| Schedulers | 1 | 2 | +1 (Reconciler) |
| Миграции | 0012 | 0017 | +5 |
| Реальных операций в проде | 0 | 547 | +547 |

### Закрытый долг

- ✅ #13 Reconciler (engine сверки) — закрыто; v1 матчит **по основанию** (номер договора), не «контрагент = хоть что-то»
- ✅ Contract + Act entities; operation_class; нормализация имён

### Новый долг / открытые вопросы

- **Ингест договоров.** Хранилище `contracts` пусто. Нужен путь документ → Contract (как у выписок: парсер + загрузчик), иначе реестру нечего закрывать. Источник — почта (основной) / GDrive (backfill).
- **3 обрезанных имени** (ИНН 7720410540/6453083366/9722049033 = `ООО (наименование не распознано)`): экстрактор схватил лишь первую строку многострочной ячейки. Фикс — разбор многострочного имени или подтяжка по ИНН из ЕГРЮЛ.
- **Предмет (этап 2).** Основание (номер договора) сделано. Остаётся предмет: не флагать легитимные разовые/материальные платежи по счёту без договора. Требует разбора назначения (subject_category).
- **Отправка алертов (коммит 5).** Флаги пока `DETECTED`; переход `REQUEST_SENT` через Outbox + Agent Caller + подавление backfill (`suppressed_by_historic`).
- **Разделение полномочий** — отдельный арх-этап: кто откуда берёт документы, кто чем алертит, кто маршрутизирует.

### Следующая сессия — варианты

1. Ингест договоров (документ → Contract) + проверка авто-закрытия флага по номеру.
