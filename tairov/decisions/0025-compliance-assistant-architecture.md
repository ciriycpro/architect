...
Accepted (23.05.2026). Карта компонентов и языковая раскладка стека комплаенс-ассистента целиком. Связывает существующий mail-stack (DEC-005..024) с business-tier (DEC-023). Открывает плейсхолдеры под будущие ADR.
...
│  • attachment-service ✓  │          │  • Inspector + Scheduler     │
│  • parser-service ✓      │          │  • Reconciler (engine)       │
│  • summary-service ✓     │          │  • Document Classifier rules │
│  • intent-tagger         │          │    (часть)                   │
│    (расширение summary)  │          │  • Contract Validator        │
│  • document-classifier   │          │    (часть: rules/status)     │
│    [future]              │          │  • Template engine           │
│  • reporting-service     │          │    (Freemarker hot-reload)   │
│    [future]              │          │  • State machines            │
│  • contract-vision       │          │  • AAA + multi-tenant        │
│    (часть: scan→signed?) │          │  • Outbox (гарантированная   │
│    в parser-service      │          │    доставка алертов)         │
│                          │          │                              │
│  ← Stateless AI tools    │          │  ← Stateful source of truth  │
└──────────────────────────┘          └──────────────────────────────┘
                                                       ↓
                                       ┌──────────────────────────┐
                                       │  TRANSPORT TIER          │
...
| **Registry** | **Java/Spring** | Planned | DEC-023 v1 | **DEC-023** |
...
- ❌ **Кривая освоения Spring Boot** для разработчика без Java-опыта — несколько недель плотной работы
...
|---|---|---|---|
| **DEC-023 v1.0** | Registry (минимум) + Inspector + Scheduler + Template engine + endpoint POST /compliance-event + интеграция с Agent Caller + Postgres на coo | DEC-025 (этот) | На горизонте 1-2 недель |
