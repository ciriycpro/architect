...
## Implementation Notes — 23.06.2026 (DEC-0030 — summary-prep service)

После санации канона 23.06.2026 (см. отдельные patches) и принятия DEC-0030 (summary-prep) число production-микросервисов вырастает с 8 до **9**:

| # | Сервис | Стек | Порт | ADR |
|---|---|---|---|---|
| 1 | agent-caller | Node.js | 3000 | DEC-0005 |
| 2 | mail-service v1.2 | Python+FastAPI | 8765 | DEC-0007, DEC-0022 |
| 3 | attachment-service v1.1 | Python+FastAPI | 8766 | DEC-0011, DEC-0022 |
| 4 | parser-service v1.1 | Python+FastAPI | 8767 | DEC-0008, DEC-024, DEC-0027 |
| 5 | summary-service v1 | Python+FastAPI | 8768 | DEC-0009 |
| 6 | orchestrator v1.2.2 | Go | 8769 | DEC-0014, DEC-0027 |
| 7 | state-service v1.0 | Go+Redis | 8770 | DEC-0021 |
| 8 | compliance-logic v0.0.7 | Java+Spring | 8771 | DEC-0023, DEC-026 |
| **9** | **summary-prep v1.0** | **Python+FastAPI** | **8772** | **DEC-0030** |

### Изменение workflow email_digest_v1 после DEC-0030

```
orchestrator (workflow email_digest_v1):
  1. fetch_mail → mail-service                        :8765
  2. download_attachments → attachment-service        :8766
  3. parse_attachments → parser-service               :8767  (raw text + warnings)
  4. distill_attachments → summary-prep    :8772   ← НОВЫЙ STEP DEC-0030
                                          (DistillResult per attachment)
  5. summary → summary-service                        :8768  (массив писем с DistillResult)
  6. notify → agent-caller                            :3000  (WhatsApp pre-alert)
  7. deliver → agent-caller                           :3000  (Telegram дайджест)
  8. SetLastAt(now) iff deliveredOK    [техдолг dedlock, см. cleanup_backlog_v2.md 6.1]
```

Шаг 4 параллельный (asyncio.gather на стороне orchestrator).

### Будущая интеграция compliance-logic (DEC-0028 Phase 2)

```
compliance-logic POST /contracts/ingest → summary-prep с contract_strictness="hard"
                                       → DistillResult обязательные поля для type=contract
                                       → contracts/{id} в БД
```

Это задел, не покрыт в MVP реализации DEC-0030.
