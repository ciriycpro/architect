...
- Минусы:
  - Логика заперта в UI, не в git → LLM-напарник не имеет контекста
...
- После часа 2 — bash-cron страховка (30 минут): cron на coo вызывает endpoints через curl/jq, доставка через `curl -X POST telegram-bot-api/sendMessage`
